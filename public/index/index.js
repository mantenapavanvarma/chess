...
$(document).ready(function () {
    ...
    function updateCapturedPiece(movement){
        const color = movement['color'];
        const capturedPieceColor = color === 'w' ? 'b' : 'w';
        state.capturedPieces[`${color}`].push(movement['captured']);
        const count = state.capturedPieces[`${color}`].reduce((acc, curr) => {
            acc[curr] = (acc[curr] || 0) + 1;
            return acc;
          }, {});
        renderCapturedPieces(getSplittedCount(count), color, capturedPieceColor)
    }

    function getSplittedCount(capturedPiecesCount){
        const finalCount = {'p': [],'r': [],'n': [],'b': [],'q': []}
        const maxCount = {'p': 8,'r': 2,'n': 2,'b': 2,'q': 1}
        for (const piece in capturedPiecesCount) {
            for (let index = 0; index < Math.floor(capturedPiecesCount[piece]/maxCount[piece]); index++) {
                console.log(capturedPiecesCount[piece]/maxCount[piece]);
                
                finalCount[piece].push(maxCount[piece]);
            }
            const remainingCount = capturedPiecesCount[piece] % maxCount[piece];
            if(remainingCount > 0){
                finalCount[piece].push(remainingCount);
            }
        }
        return finalCount;
    }

    function renderCapturedPieces(capturedPiecesCount, color, capturedPieceColor) {
        const isFlippedBoard = $('.board.flipped').length > 0;
        let orientation = null;
        if (isFlippedBoard) {
            orientation = color === 'w' ? 'top' : 'bottom';
        } else {
            orientation = color === 'w' ? 'bottom' : 'top';
        }
        const $capturedPiecesContainer = $(`.player-${orientation}-elemination-container`);
        $capturedPiecesContainer.html('');
        for (const piece in capturedPiecesCount) {
            const count = capturedPiecesCount[piece];
            for (let index = 0; index < count.length; index++) {
                const finalCount = count[index];
                let $pieceElement = null
                if (piece === 'q'){
                    $pieceElement = $(`<span class="captured-pieces-cpiece captured-pieces-${capturedPieceColor}-${pieceMapper[piece]}"></span>`);
                }else{
                    $pieceElement = $(`<span class="captured-pieces-cpiece captured-pieces-${capturedPieceColor}-${finalCount}-${pieceMapper[piece]}"></span>`);
                }
                $capturedPiecesContainer.append($pieceElement);
            }
        }
    }
